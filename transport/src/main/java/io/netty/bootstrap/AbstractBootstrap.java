...
public abstract class AbstractBootstrap<B extends AbstractBootstrap<B, C>, C extends Channel> implements Cloneable {
    ...
    private ChannelFuture doBind(final SocketAddress localAddress) {
        //创建并注册一个Channel对象，因为注册是异步的过程，所以返回一个ChannelFuture对象
        ...
        if (regFuture.isDone()) {   //如果已经创建好了，直接处理绑定
            // At this point we know that the registration was complete and successful.
            ChannelPromise promise = channel.newPromise();
            doBind0(regFuture, channel, localAddress, promise);
            return promise;
        } else {    //如果未创建好，则需要创建一个PendingRegistrationPromise
    ...
    /**
     * 创建Channel、ChannelFuture，并将Channel注册到EventLoopGroup中
     *
    ...
    private static void doBind0(
        ...
        // 这个方法在channelRegistered()被触发之前调用。给用户的handlers在channelRegistered()方法的实现中设置pipeline一个机会。
        //
        // 调用 EventLoop 执行 Channel 的端口绑定逻辑。但是，实际上当前线程已经是 EventLoop 所在的线程了，为何还要这样操作呢？答案在英语注释。
